...
from lib.implicit_flow import ACT_FNS, ImplicitFlow
from lib.resflow import ACT_FNS, ResidualFlow
from lib.resxflow import ACT_FNS, ResidualXFlow
# from lib.mmlocalflow import ACT_FNS, MMLocalFlow
# from lib.localvitflow import ACT_FNS, LocalViTFlow
# from lib.resmlpflow import ACT_FNS, ResidualMLPFlow
from lib.convnextflow import ACT_FNS, ConvNextFlow
from lib.maxvitflow import ACT_FNS, MaxViTFlow
from lib.vanflow import ACT_FNS, VANFlow
...
# ---------------------------------- Arguments ---------------------------------- #
parser = argparse.ArgumentParser('Manifold Matching via Deep Metric Learning for Generative Flow-based Model'
                                 'with The Global and Local Receptive Fields.',  add_help=False)
parser.add_argument(
...
parser.add_argument('--model', type=str, default='ResidualFlow')
...
parser.add_argument('--layer_scaler_init_value', type=float, default=1e-6)
...
# ---------------------- Flow-based Model for Generate Data ---------------------- #
if args.model == 'ResidualFlow':
    model = ResidualFlow(
        input_size,
    ...
    model = ResidualXFlow(
        ...
        neumann_grad=args.neumann_grad,
        grad_in_forward=args.mem_eff,
        first_resxblock=True,
        learn_p=args.learn_p,
        classification=args.task in ['classification', 'hybrid'],
        classification_hdim=args.cdim,
        n_classes=n_classes,
        block_type='resxblock',
    )
elif args.model == 'ImplicitFlow':
    model = ImplicitFlow(
        input_size,
...
if args.model == 'ConvNextFlow':
    model = ConvNextFlow(
        ...
        layernorm=True,
        drop_p=args.drop_p,
        layer_scaler_init_value=args.layer_scaler_init_value,
        fc=args.fc,
        coeff=args.coeff,
        vnorms=args.vnorms,
        n_lipschitz_iters=args.n_lipschitz_iters,
...
def estimator_moments(model, baseline=0):
    ...
    if args.model == 'ResidualFlow':
        ...
        for m in model.modules():
            if isinstance(m, layers.iResXBlock):
                avg_first_moment += m.last_firmom.item()
                avg_second_moment += m.last_secmom.item()
    elif args.model == 'ImplicitFlow':
        for m in model.modules():
            if isinstance(m, layers.impBlock):
                avg_first_moment += m.last_firmom.item()
                avg_second_moment += m.last_secmom.item()
    elif args.model == 'ConvNextFlow':
        for m in model.modules():
            if isinstance(m, layers.ConvxBlock):
                avg_first_moment += m.last_firmom.item()
                avg_second_moment += m.last_secmom.item()
    elif args.model == 'CoAtFlow':
        for m in model.modules():
            if isinstance(m, layers.CoAtBlock):
                avg_first_moment += m.last_firmom.item()
                avg_second_moment += m.last_secmom.item()
    elif args.model == 'MMGbLcFlow':
        for m in model.modules():
            if isinstance(m, layers.MMGbLcBlock):
                avg_first_moment += m.last_firmom.item()
                avg_second_moment += m.last_secmom.item()
    ...
    if args.model == 'ResidualFlow':
        for m in model.modules():
            ...
            if isinstance(m, base_layers.LopConv2d) or isinstance(m, base_layers.LopLinear):
                lipschitz_constants.append(m.scale)
    elif args.model == 'ResidualXFlow':    # 仅考虑了深度卷积规范层与线性规范层
        for m in model.modules():
            if isinstance(m, base_layers.SpectralNormDWConv2d) or isinstance(m, base_layers.SpectralNormLinear):
                lipschitz_constants.append(m.scale)
            if isinstance(m, base_layers.InducedNormDWConv2d) or isinstance(m, base_layers.InducedNormLinear):
                lipschitz_constants.append(m.scale)
            if isinstance(m, base_layers.LopDWConv2d) or isinstance(m, base_layers.LopLinear):
                lipschitz_constants.append(m.scale)
    elif args.model == 'ImplicitFlow':
        for m in model.modules():
            if isinstance(m, base_layers.SpectralNormConv2d) or isinstance(m, base_layers.SpectralNormLinear):
...
############################ The Main of Flow-based Genertive Model ############################
def main(model):
...
"""
最好别陷入到CNN、Trans、MLP其中的一个，三足鼎立，前沿会一直在变化，还是走融合CNN+Attention这条路比较合适。
fusing two key insight:depthwise convolution + self-attention.
gaining the global and local receptive fields for attention.
Therefore, the name of our method is MMGbLcFlow:
Manifold Matching via Deep Metric Learning for Generative Flow-based Model with The Global and Local Receptive Fields.
*********** decompose the task of our method.
Take your time, make a legible route:
ResNet -> ResNeXt -> ConvNeXt -> VAN -> CoAtNet -> MaxViT

最后统一将方法名字修改为MMGbLcFlow
