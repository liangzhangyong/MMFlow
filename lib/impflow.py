...
class ImplicitFlow(nn.Module):
    ...
    def __init__(
        ...
        self.first_impblock = first_impblock
    ...
    def _build_net(self, input_size):
        _, c, h, w = input_size
        transforms = []
        _stacked_blocks = StackedImplicitBlocks if self.block_type == 'impblock' else StackedCouplingBlocks
        for i in range(self.n_scale):
            transforms.append(
                _stacked_blocks(
                    ...
                    first_impblock=self.first_impblock and (i == 0),
...
class StackedImplicitBlocks(layers.SequentialFlow):
    ...
    def __init__(
        ...
        def _impblock(initial_size, fc, idim=idim, first_impblock=False):
            if fc:
                return layers.impBlock(
                    FCNet(
                ...
                def build_nnet():
                    ...
                    nnet = []
                    if not first_impblock and preact:
        ...
        if first_impblock and actnorm: chain.append(_actnorm(initial_size, fc))
        if first_impblock and fc_actnorm: chain.append(_actnorm(initial_size, True))

        if squeeze:
            c, h, w = initial_size
            for i in range(n_blocks):
                if quadratic: chain.append(_quadratic_layer(initial_size, fc))
                chain.append(_impblock(initial_size, fc, first_impblock=first_impblock and (i == 0)))
            ...
            if fc_end:
                for _ in range(fc_nblocks):
                    chain.append(_impblock(initial_size, True, fc_idim))
                    if actnorm or fc_actnorm: chain.append(_actnorm(initial_size, True))
        super(StackedImplicitBlocks, self).__init__(chain)
    ...
    def __init__(
        ...
        if squeeze:
            ...
            for _ in range(n_blocks):
                if quadratic: chain.append(_quadratic_layer(initial_size, fc))
                chain.append(_impblock(initial_size, fc))
                if actnorm: chain.append(_actnorm(initial_size, fc))
                if fc_actnorm: chain.append(_actnorm(initial_size, True))
            # Use four fully connected layers at the end.
            if fc_end:
