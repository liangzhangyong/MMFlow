"""
Lib约束，i-ResFlow与ResFlow最重要的创新点---引入范数约束，将流模型压缩至可逆空间内

Soft Spectral Normalization (not enforced, only <= coeff) for Conv2D and Attention layers
...
from .mixed_lipschitz import InducedNormLinear, InducedNormConv2d, InducedNormDWConv2d, InducedNormAttention

__all__ = ['SpectralNormLinear', 'SpectralNormConv2d', 'SpectralNormDWConv2d', 'SpectralNormAttention',
           'LopLinear', 'LopConv2d', 'LopDWConv2d', 'get_linear', 'get_conv2d', 'get_dwconv2d']


# --------------------------------------------------------------------------------------- #
class SpectralNormLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=True, coeff=0.97, n_iterations=None, atol=None, rtol=None,
                 **unused_kwargs):
        del unused_kwargs
...
class SpectralNormConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=True,
                 coeff=0.97, n_iterations=None, atol=None, rtol=None,
                 **unused_kwargs):
        del unused_kwargs
        ...
        if update:
            with torch.no_grad():
                ...
                for _ in range(n_iterations):
                    old_u = u.clone()
                    old_v = v.clone()
                    v_s = F.conv_transpose2d(u.view(self.out_shape), weight, stride=self.stride,
...
# --------------------------------------------------------------------------------------- #
# 深度卷积的谱规范化(Lip(p)<1)
class SpectralNormDWConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups,
                 bias=True, coeff=0.97, n_iterations=None, atol=None, rtol=None, **unused_kwargs):
        del unused_kwargs
        super(SpectralNormDWConv2d, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = _pair(kernel_size)
        self.stride = _pair(stride)
    ...
    def _initialize_u_v(self):
        if self.kernel_size == (1, 1):
            ...
            with torch.no_grad():
                ...
                v = F.normalize(torch.randn(num_input_dim).to(self.weight), dim=0, eps=1e-12)
                # forward call to infer the shape
                u = F.conv2d(v.view(1, c, h, w), self.weight, stride=self.stride, padding=self.padding,
        ...
        if update:
            with torch.no_grad():
                ...
                for _ in range(n_iterations):
                    ...
                    old_v = v.clone()
                    v_s = F.conv_transpose2d(u.view(self.out_shape), weight, stride=self.stride,
                                             padding=self.padding, groups=self.groups, output_padding=0)
                    v = F.normalize(v_s.view(-1), dim=0, out=v)
                    u_s = F.conv2d(v.view(1, c, h, w), weight, stride=self.stride, padding=self.padding,
                                   groups=self.groups, bias=None)
                    u = F.normalize(u_s.view(-1), dim=0, out=u)
                    itrs_used = itrs_used + 1
                    if atol is not None and rtol is not None:
                ...
                if itrs_used > 0:
                    u = u.clone()
                    v = v.clone()
        weight_v = F.conv2d(v.view(1, c, h, w), weight, stride=self.stride, padding=self.padding, groups=self.groups,
                            bias=None)
        weight_v = weight_v.view(-1)
        sigma = torch.dot(u.view(-1), weight_v)
        with torch.no_grad():
    ...
    def forward(self, input):
        if not self.initialized: self.spatial_dims.copy_(torch.tensor(input.shape[2:4]).to(self.spatial_dims))
        weight = self.compute_weight(update=self.training)
        return F.conv2d(input, weight, self.bias, self.stride, self.padding, 1, self.groups)
...
# --------------------------------------------------------------------------------------- #
# 自注意力的谱规范化(Lip(p)<1)
class SpectralNormAttention(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups,
                 bias=True, coeff=0.97, n_iterations=None, atol=None, rtol=None,
                 **unused_kwargs):
        del unused_kwargs
        super(SpectralNormAttention, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = _pair(kernel_size)
        self.stride = _pair(stride)
...
class LopDWConv2d(nn.Conv2d):
    ...
    def __init__(self,
                 ...
                 **unused_kwargs):
        del unused_kwargs
        super(LopDWConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, groups, bias)
        self.coeff = coeff
        self.domain = domain
        self.codomain = codomain
        self.local_constraint = local_constraint
    ...
    def extra_repr(self):
        s = super(LopDWConv2d, self).extra_repr()
        return s + ', coeff={}, domain={}, codomain={}, local={}'.format(
            self.coeff, self.domain, self.codomain, self.local_constraint)


# --------------------------------------------------------------------------------------- #
class LopAttention(nn.Conv2d):
    """Lipschitz constant of self-attention defined using operator norms.
       Lip<1，但不能等于0，可以等于1，最后一层必须小于1。
    """
    def __init__(self,
                 in_channels,
                 ...
                 **unused_kwargs):
        del unused_kwargs
        super(LopAttention, self).__init__(in_channels, out_channels, kernel_size, stride, padding, bias)
        self.coeff = coeff
        self.domain = domain
        self.codomain = codomain
        self.local_constraint = local_constraint
    ...
    def extra_repr(self):
        s = super(LopAttention, self).extra_repr()
        return s + ', coeff={}, domain={}, codomain={}, local={}'.format(
            self.coeff, self.domain, self.codomain, self.local_constraint)
...
class LipNormLinear(nn.Linear):
    ...
    def __init__(self,
        ...
        self.domain = domain
        self.codomain = codomain
        self.local_constraint = local_constraint  # Llb<1约束
        max_across_input_dims, self.norm_type = operator_norm_settings(self.domain, self.codomain)
        self.max_across_dim = 1 if max_across_input_dims else 0
...
# --------------------------------------------------------------------------------------- #
class LipNormDWConv2d(nn.Conv2d):
    """Lipschitz constant defined using operator norms.
       Lip<1，但不能等于0，可以等于1，最后一层必须小于1。
    """
    def __init__(self,
                 ...
                 **unused_kwargs):
        del unused_kwargs
        super(LipNormDWConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, groups, bias)
        self.coeff = coeff
        self.domain = domain
        self.codomain = codomain
        self.local_constraint = local_constraint
    ...
    def extra_repr(self):
        s = super(LipNormDWConv2d, self).extra_repr()
        return s + ', coeff={}, domain={}, codomain={}, local={}'.format(
            self.coeff, self.domain, self.codomain, self.local_constraint)


# --------------------------------------------------------------------------------------- #
class LipNormAttention(nn.Conv2d):
    """Lipschitz constant defined using operator norms.
       Lip<1，但不能等于0，可以等于1，最后一层必须小于1。
    """
    def __init__(self,
                 ...
                 **unused_kwargs):
        del unused_kwargs
        super(LipNormAttention, self).__init__(in_channels, out_channels, kernel_size, stride, padding, bias)
        self.coeff = coeff
        self.domain = domain
        self.codomain = codomain
        self.local_constraint = local_constraint
    ...
    def extra_repr(self):
        s = super(LipNormAttention, self).extra_repr()
        return s + ', coeff={}, domain={}, codomain={}, local={}'.format(
            self.coeff, self.domain, self.codomain, self.local_constraint)
...
def get_dwconv2d(in_channels, out_channels, kernel_size, stride, padding, groups, bias=True,
                 coeff=0.97, domain=None, codomain=None, **kwargs):
    _dwconv2d = InducedNormDWConv2d
    if domain == 1:
        if codomain in [1, 2, float('inf')]:
            _dwconv2d = LopDWConv2d
    elif codomain == float('inf'):
        if domain in [2, float('inf')]:
            _dwconv2d = LopDWConv2d
    return _dwconv2d(in_channels, out_channels, kernel_size, stride, padding, groups, bias, coeff, domain, codomain,
                     **kwargs)


def get_attention(in_channels, out_channels, kernel_size, stride, padding, bias=True,
                  coeff=0.97, domain=None, codomain=None, **kwargs):
    _attention = InducedNormAttention
    if domain == 1:
        if codomain in [1, 2, float('inf')]:
            _attention = LopAttention
    elif codomain == float('inf'):
        if domain in [2, float('inf')]:
            _attention = LopAttention
    return _attention(in_channels, out_channels, kernel_size, stride, padding, bias, coeff, domain, codomain, **kwargs)


##########################################################################
if __name__ == '__main__':
    # UnitTest
    print('test for spectral normalization')
