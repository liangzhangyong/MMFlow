...
class InvertibleConv2d(nn.Module):
    ...
    def inverse(self, y, logpy=None):  # F^-1
...
# -------------------------------------------------------------------------------------- #
class InvertibleDWConv2d(nn.Module):
    def __init__(self, dim):
        super(InvertibleDWConv2d, self).__init__()
        self.dim = dim
        self.weight = nn.Parameter(torch.eye(dim)[torch.randperm(dim)])

    def forward(self, x, logpx=None):
        y = F.conv2d(x, self.weight.view(self.dim, self.dim, 1, 1), groups=self.dim)
        if logpx is None:
            return y
        else:
            return y, logpx - self._logdetgrad.expand_as(logpx) * x.shape[2] * x.shape[3]
    ...
    def inverse(self, y, logpy=None):  # F^-1
        x = F.conv2d(y, self.weight.inverse().view(self.dim, self.dim, 1, 1), groups=self.dim)
        if logpy is None:
            return x
        else:
            return x, logpy + self._logdetgrad.expand_as(logpy) * x.shape[2] * x.shape[3]
...
"""
the inverse block of our model, include the InvertibleLinear, InvertibleConv2d, InvertibleDWConv2d,
InvertibleMBConv, InvertibleBlockAtten, InvertibleGridAtten.
"""
