...
__all__ = ['InducedNormLinear', 'InducedNormConv2d', 'InducedNormDWConv2d']


class InducedNormLinear(nn.Module):
    def __init__(self,
                 in_features, out_features, bias=True, coeff=0.97, domain=2, codomain=2, n_iterations=None,
                 atol=None, rtol=None, zero_init=False, **unused_kwargs):
        del unused_kwargs
...
class InducedNormConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=True,
                 coeff=0.97, domain=2, codomain=2, n_iterations=None, atol=None, rtol=None,
                 **unused_kwargs):
        del unused_kwargs
        ...
        with torch.no_grad():
            ...
            if self.kernel_size == (1, 1):
                ...
                with torch.no_grad():
                    ...
                    u = F.conv2d(self.v.view(1, c, h, w), self.weight, stride=self.stride,
                                 padding=self.padding, bias=None)
    ...
    def compute_one_iter(self):
        ...
        if self.kernel_size == (1, 1):
            ...
            v_s = F.conv_transpose2d(u.view(out_shape), weight, stride=self.stride,
...
# -------------------------------------------------------------------------------------- #
class InducedNormDWConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups, bias=True,
                 coeff=0.97, domain=2, codomain=2, n_iterations=None, atol=None, rtol=None,
                 **unused_kwargs):
        del unused_kwargs
        super(InducedNormDWConv2d, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = _pair(kernel_size)
        self.stride = _pair(stride)
        ...
        with torch.no_grad():
            ...
            if self.kernel_size == (1, 1):
                ...
                with torch.no_grad():
                    ...
                    # forward call to infer the shape
                    u = F.conv2d(self.v.view(1, c, h, w), self.weight, stride=self.stride,
                                 padding=self.padding, groups=self.groups, bias=None)
                    num_output_dim = u.shape[0] * u.shape[1] * u.shape[2] * u.shape[3]
                    # overwrite u with random init
                    self.u.resize_(num_output_dim).normal_(0, 1)
                    self.u.copy_(normalize_u(self.u, codomain))
    ...
    def compute_one_iter(self):
        ...
        if self.kernel_size == (1, 1):
            ...
            weight = self.weight.detach()
            c, h, w = self.in_channels, int(self.spatial_dims[0].item()), int(self.spatial_dims[1].item())
            u_s = F.conv2d(v.view(1, c, h, w), weight, stride=self.stride,
                           padding=self.padding, groups=self.groups, bias=None)
            out_shape = u_s.shape
            u = normalize_u(u_s.view(-1), codomain)
            v_s = F.conv_transpose2d(u.view(out_shape), weight, stride=self.stride,
            ...
            v = normalize_v(v_s.view(-1), domain)
            weight_v = F.conv2d(v.view(1, c, h, w), weight, stride=self.stride,
                                padding=self.padding, groups=self.groups, bias=None)
            return torch.dot(u.view(-1), weight_v.view(-1))
    ...
    def forward(self, input):
        if not self.initialized: self.spatial_dims.copy_(torch.tensor(input.shape[2:4]).to(self.spatial_dims))
        weight = self.compute_weight(update=False)
        return F.conv2d(input, weight, self.bias, self.stride, self.padding, 1, 1)  # self.groups
...
# -------------------------------------------------------------------------------------- #
class InducedNormAttention(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=True,
                 coeff=0.97, domain=2, codomain=2, n_iterations=None, atol=None, rtol=None,
                 **unused_kwargs):
        del unused_kwargs
        super(InducedNormAttention, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = _pair(kernel_size)
        self.stride = _pair(stride)
...
def asym_squash(x):
    return torch.tanh(-leaky_elu(-x + 0.5493061829986572)) * 2 + 3     # 如何定义的这个数字
...
if __name__ == '__main__':
    ...
    # m = InducedNormDWConv2d(10, 2, 3, 1, 1, 1, atol=1e-3, rtol=1e-3, domain=p, codomain=p)
    W = m.compute_weight()
