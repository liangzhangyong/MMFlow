...
# -------------------------------------------------------------------------------------- #
class iConvxBlock(nn.Module):
    def __init__(self,
                 nnet,
                 geom_p=0.5,
                 lamb=2.,
                 n_power_series=None,  # uses a biased approximation to logdet.
                 exact_trace=False,  # False, uses a Hutchinson trace estimator. True, computes the exact full Jacobian.
                 brute_force=False,  # Computes the exact logdet
                 n_samples=1,
                 n_exact_terms=2,
                 n_dist='geometric',
                 neumann_grad=True,
        ...
        with torch.enable_grad():
            ...
            if not self.exact_trace:
                ...
                if self.training and self.grad_in_forward:
                    g, logdetgrad = mem_eff_wrapper(estimator_fn, self.nnet, x, n_power_series,
                                                    vareps, coeff_fn, self.training)
                else:
                    x = x.requires_grad_(True)
                    g = self.nnet(x)
