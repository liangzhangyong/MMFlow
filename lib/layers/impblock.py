...
class impBlock(nn.Module):
    ...
    class Backward(Function):
        ...
        in the backward pass. Essentially a wrapper that provides backprop for the `impBlock` class.
        You should use this inner class in impBlock's forward() function by calling:
    ...
    def _logdetgrad(self, z, x):
        ...
        with torch.enable_grad():
            ...
            if not self.exact_trace:
                ...
                vareps_x = torch.distributions.bernoulli.Bernoulli(torch.Tensor([0.5])).sample(x.shape).reshape(
                    x.shape).to(x) * 2 - 1
                vareps_z = torch.distributions.bernoulli.Bernoulli(torch.Tensor([0.5])).sample(z.shape).reshape(
                    z.shape).to(z) * 2 - 1
                ...
                if self.training and self.grad_in_forward:
                    ...
                    logdetgrad = logdet_x - logdet_z  # 同时引入x与z，对称求解
                else:
                    x = x.requires_grad_(True)
                    z = z.requires_grad_(True)
                    Fx = self.nnet_x(x)
                ...
                logdetgrad = logdetJx - logdetJz  # 同时引入x与z，对称求解
    ...
    def extra_repr(self):
        return 'dist={}, n_samples={}, n_power_series={}, neumann_grad={}, exact_trace={}, brute_force={}, neumann_grad={}, grad_in_forward={}'.format(
            ...
            self.neumann_grad, self.grad_in_forward
        )
...
def batch_jacobian(g, x, create_graph=True):
    ...
    for d in range(g.shape[1]):
        jac.append(
            torch.autograd.grad(torch.sum(g[:, d]), x, create_graph=create_graph)[0].view(x.shape[0], 1, x.shape[1]))
