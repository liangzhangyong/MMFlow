...
class ResidualXFlow(nn.Module):
    def __init__(
        ...
        super(ResidualXFlow, self).__init__()
        ...
        self.first_resxblock = first_resxblock
    ...
    def _build_net(self, input_size):
        _, c, h, w = input_size
        transforms = []
        _stacked_blocks = StackediResXBlocks if self.block_type == 'resxblock' else StackedCouplingBlocks
        for i in range(self.n_scale):
            transforms.append(
                _stacked_blocks(
                    ...
                    first_resxblock=self.first_resxblock and (i == 0),
    ...
    def build_multiscale_classifier(self, input_size):
        ...
        for i, hshape in enumerate(hidden_shapes):
            classification_heads.append(
                nn.Sequential(
                    nn.Conv2d(hshape[1], self.classification_hdim, kernel_size=3,
                              stride=1, padding=1),
...
class StackediResXBlocks(layers.SequentialFlow):
    def __init__(
            self,
            initial_size,
            idim,
        ...
        def _resxblock(initial_size, fc, idim=idim, first_resxblock=False):
            if fc:
                return layers.iResXBlock(
                    FCNet(
                ...
                for i, k in enumerate(ks[1:-1]):
                    nnet.append(
                        _lipschitz_layer_dwconv(fc)(
                            idim, idim, k, 1, k // 2, groups=idim, coeff=coeff, n_iterations=n_lipschitz_iters,
        ...
        if first_resxblock and actnorm: chain.append(_actnorm(initial_size, fc))
        if first_resxblock and fc_actnorm: chain.append(_actnorm(initial_size, True))

        if squeeze:
            c, h, w = initial_size
            for i in range(n_blocks):
                if quadratic: chain.append(_quadratic_layer(initial_size, fc))
                chain.append(_resxblock(initial_size, fc, first_resxblock=first_resxblock and (i == 0)))
            ...
            for _ in range(n_blocks):
                if quadratic: chain.append(_quadratic_layer(initial_size, fc))
                chain.append(_resxblock(initial_size, fc))
                if actnorm: chain.append(_actnorm(initial_size, fc))
                if fc_actnorm: chain.append(_actnorm(initial_size, True))
            # Use four fully connected layers at the end.
            if fc_end:
                for _ in range(fc_nblocks):
                    chain.append(_resxblock(initial_size, True, fc_idim))
                    if actnorm or fc_actnorm: chain.append(_actnorm(initial_size, True))

        super(StackediResXBlocks, self).__init__(chain)


class FCNet(nn.Module):
    def __init__(self, input_shape, idim, lipschitz_layer_conv, lipschitz_layer_dwconv, nhidden, coeff, domains,
                 codomains, n_iterations, activation_fn, preact, dropout, sn_atol, sn_rtol, learn_p, div_in=1):
        ...
        # the first layer
        nnet.append(
            lipschitz_layer_conv(last_dim, idim) if lipschitz_layer_conv == nn.Linear else lipschitz_layer_conv(
                last_dim, idim, coeff=coeff, n_iterations=n_iterations, domain=domains[0], codomain=codomains[0],
                atol=sn_atol, rtol=sn_rtol))
        nnet.append(ACT_FNS[activation_fn](True))
        last_dim = idim
        # the second layer
        nnet.append(
            lipschitz_layer_dwconv(last_dim, idim) if lipschitz_layer_dwconv == nn.Linear else lipschitz_layer_dwconv(
                last_dim, idim, groups=idim, coeff=coeff, n_iterations=n_iterations, domain=domains[1], codomain=codomains[1],
                atol=sn_atol, rtol=sn_rtol))
        nnet.append(ACT_FNS[activation_fn](True))
        if dropout: nnet.append(nn.Dropout(dropout, inplace=True))
        # the third layer
        nnet.append(
            lipschitz_layer_conv(last_dim, dim) if lipschitz_layer_conv == nn.Linear else lipschitz_layer_conv(
...
# don't compare with coupling block and our block.
    ...
    def __init__(
        ...
        def _resxblock(initial_size, fc, idim=idim, first_resxblock=False):
            if fc:
                ...
                nnet.append(ACT_FNS[activation_fn](True))
                for i, k in enumerate(ks[1:-1]):
                    nnet.append(_weight_layer(fc)(idim, idim, k, 1, k // 2, groups=idim))
                    if batchnorm: nnet.append(layers.MovingBatchNorm2d(idim))
                    nnet.append(ACT_FNS[activation_fn](True))
                if dropout: nnet.append(nn.Dropout2d(dropout, inplace=True))
...
"""
ResNet整个实例化网络由stem layer(降低维度) + main layer(提取featuremap) + classifier(FC层)组成。
对于block类中，由1x1+3x3+1x1卷积组成。
ResNeXt在深度与宽度的基础上引入了‘基数-cardinality’这一关键因子。
标准ResNet中的expansion=4，而ResNeXt中的expansion=2；
基数-cardinality对应卷积里面的group，在基线3x3Conv2d中引入group=32
定义block类，由1x1+3x3+1x1卷积组成，仅需要在3x3conv2d中修改group与expansion参数。
实例化的网络仍然由stem layer + main layer + classifier组成。
"""
